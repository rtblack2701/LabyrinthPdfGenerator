...
def main():
    ...
    run_script(["python", "-m", "scripts.refinement_scripts.refined_fetch"])
    run_script(["python", "-m", "scripts.refinement_scripts.refined_pdf_filler"])
    
    if check_for_empty_data('final_output.json'):
        run_script(["python", "-m", "scripts.post_process_scripts.generated_file_removal"])
        print("No induction forms to process.")
        return
    
    run_script(["python", "-m", "scripts.refinement_scripts.refined_pdf_gen"])
